          ...
          {[1, 2, 3].map((index) => (
            <div key={index} className="space-y-4">
              ...
              <motion.div 
                ...
                animate={{ opacity: [0.5, 1, 0.5] }}
                transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut", delay: index * 0.2 + 0.1 }}
              />
              <motion.div 
                className="h-4 bg-slate-100 rounded-lg w-full"
                animate={{ opacity: [0.5, 1, 0.5] }}
                transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut", delay: index * 0.2 + 0.2 }}
              />
              <motion.div 
                className="h-4 bg-slate-100 rounded-lg w-5/6"
                animate={{ opacity: [0.5, 1, 0.5] }}
                transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut", delay: index * 0.2 + 0.3 }}
              />
            </div>
          ))}
        </div>
