...
const CardBackground = ({ type }: { type: string }) => {
  switch (type) {
    ...
    case 'assessments':
      return (
        <>
          <div className="absolute inset-0 bg-brand-700 transition-colors group-hover:bg-brand-800" />
          <AssessmentWireframe />
        </>
      );
    case 'playbooks':
    default:
      return (
        <>
          <div className="absolute inset-0 bg-slate-900 transition-colors group-hover:bg-slate-950" />
          <PlaybookWireframe />
        </>
      );
  }
...
export default function Resources() {
  ...
  const [categoriesList, setCategoriesList] = useState<ResourceCategory[]>([]);

  const filteredCategories = activeCategory
    ? categoriesList.filter(
        (category) =>
          category.id.toLowerCase() === activeCategory.toLowerCase() ||
  ...
  useEffect(() => {
    ...
    getContentRepository()
      ...
      .then((categories) => {
        if (mounted) setCategoriesList(categories);
      });
    return () => {
      mounted = false;
    };
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-32 pb-24 lg:pt-40 lg:pb-32">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-display font-extrabold text-slate-900 tracking-tight mb-6 leading-[1.1]">
            Executive Resources
          </h1>
            ...
            <div key={category.id} className="relative">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                <motion.div 
                  ...
                  className="max-w-2xl"
                >
                  <h2 className="text-2xl md:text-3xl font-display font-extrabold text-slate-900 mb-2 tracking-tight">
                    {category.title}
                  </h2>
                ...
                >
                  <Link 
                    ...
                    className="inline-flex items-center space-x-2 bg-slate-900 text-white hover:bg-brand-500 hover:text-slate-950 px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-300 shadow-md hover:shadow-brand-500/20 focus:ring-4 focus:ring-brand-200"
                  >
                    <span>See all</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {category.items.map((resourceItem, index) => (
                  <Link
                    key={resourceItem.id || resourceItem.slug || index}
                    to={`/resources/${category.id}/${resourceItem.id || resourceItem.slug || index + 1}`}
                  >
                    <motion.div
                      ...
                      className="group relative rounded-3xl overflow-hidden aspect-[3/4] shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer flex flex-col h-full border border-slate-200/50"
                    >
                      <CardBackground type={category.id} />
                      
                      <div className="relative z-10 p-8 flex flex-col h-full">
                        <div className="text-white/80 font-bold text-[10px] uppercase mb-4 tracking-widest">
                        ...
                        <div className="mt-auto">
                          <div className="w-10 h-10 rounded-xl bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20 group-hover:bg-white group-hover:text-slate-900 text-white transition-all duration-300 shadow-sm">
