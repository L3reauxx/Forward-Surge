...
const ProgramCard: React.FC<{ program: Program; index: number }> = ({ program, index }) => {
  ...
  return (
    <motion.div 
      ...
      className="bg-white rounded-3xl p-8 lg:p-12 shadow-xl shadow-slate-100 border border-slate-200 relative overflow-hidden group hover:shadow-2xl hover:shadow-brand-100/50 hover:-translate-y-1 hover:border-brand-300 transition-all duration-300"
    >
      <motion.div 
        style={{ x: springX, y: springY }}
        className="absolute top-0 right-0 w-[400px] h-[400px] bg-brand-50 rounded-full -mr-20 -mt-20 opacity-50 blur-3xl pointer-events-none transition-colors group-hover:bg-brand-100/60"
      />
      ...
      <div className="grid lg:grid-cols-12 gap-12 relative z-10">
        <div className="lg:col-span-5 space-y-6">
          ...
          <div>
            <h2 className="text-3xl md:text-4xl font-display font-extrabold text-slate-900 tracking-tight">{program.title}</h2>
            <h3 className="text-xs font-bold text-brand-700 mt-2 uppercase tracking-widest">{program.subtitle}</h3>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-8 flex flex-col justify-center">
          ...
          {program.overviewList && program.overviewList.length > 0 && (
            <div className="space-y-4 pt-6 border-t border-slate-200">
              {program.overviewList.map((overviewItem, index) => (
                <div key={index} className="flex space-x-3 items-start">
                  <div className="flex-shrink-0 mt-2">
                    <div className="w-2 h-2 rounded-full bg-brand-500"></div>
                  </div>
                  <div>
                    <h4 className="text-base font-bold text-slate-800 leading-relaxed">{overviewItem}</h4>
                  </div>
                </div>
              ))}
            </div>
...
export default function Programs() {
  ...
  useEffect(() => {
    ...
    getContentRepository()
      .getPrograms()
      .then((programsData) => {
        if (mounted) setProgramsList(programsData);
      });
    return () => {
      mounted = false;
    };
  ...
  return (
    <div className="bg-white">
      <section className="stripe-gradient py-20 lg:py-28 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <motion.div
            ...
            className="inline-flex items-center space-x-2 bg-white text-brand-700 px-3.5 py-1.5 rounded-full mb-8 text-[10px] font-bold tracking-widest uppercase border border-slate-200 shadow-sm"
          >
          ...
          <motion.h1 
            ...
            className="text-4xl md:text-5xl lg:text-6xl font-display font-extrabold tracking-tight mb-8 text-slate-900 leading-[1.1] flex flex-wrap justify-center gap-x-2"
          >
            {"Aligning People Strategies".split(" ").map((word, index) => (
              <motion.span 
                key={index} 
                variants={{
      ...
      <section className="py-20 lg:py-28 bg-slate-50 border-t border-slate-200">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <h2 className="text-3xl md:text-4xl font-display font-extrabold text-slate-900 mb-4 tracking-tight leading-[1.15]">
      ...
      <section className="py-20 lg:py-28 stripe-gradient border-t border-slate-200">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          >
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-display font-extrabold text-slate-900 mb-6 tracking-tight leading-[1.15]">
              Which program is right for your team?
            </h2>
            <p className="text-lg text-slate-600 mb-10 font-medium max-w-2xl mx-auto leading-relaxed">
